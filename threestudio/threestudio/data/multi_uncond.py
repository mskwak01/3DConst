...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.num_multiview > 0:
            ...
            if elevation_disp_config == "same":
                elevation = elevation[...,None].repeat(1, 1 + self.num_multiview).reshape(-1,1).squeeze()
                elevation_deg = elevation / math.pi * 180.0
                
            else:
                print("No config!")
            
            # Azimuth degress / radians
            
            multi_azim_degs = azimuth_deg[...,None] + disp_degs[None,...]
...
@register("random-multiview-datamodule")
