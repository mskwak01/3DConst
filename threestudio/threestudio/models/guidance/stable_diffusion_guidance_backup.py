...
#         # fin_sim_maps = torch.cat((-torch.ones_like(sim)[None,...], valid_sim[:2]), dim=0).squeeze()
...
#             masks = torch.cat((d_1[None,...], depth_masks[:2]), dim=0).squeeze()
#             fin_mag_maps = (1 - masks) * -1 + masks * fin_mag_maps
        
#         else:
#             fin_mag_maps = None
        
#         return valid_sim, fin_sim_maps, fin_mag_maps
...
#         valid_sim, grad_sim, grad_mag = self.warp_visualizer(grad, projections, depth_masks, re_dict, vis_magnitude=False)
#         return_dict['grad_sim'] = valid_sim
...
#                 valid_sim, rep_sim, rep_mag = self.warp_visualizer(guide_utils[key], projections, depth_masks, re_dict, vis_magnitude=True)
...
#             grad_clip = grad_sim * grad_origin.clamp(-self.grad_clip_val, self.grad_clip_val) + (1 - grad_sim) * grad_origin

#             # loss = SpecifyGradient.apply(latents, grad)
#             # SpecifyGradient is not straghtforward, use a reparameterization trick instead
#             target = (latents - grad_clip).detach()
