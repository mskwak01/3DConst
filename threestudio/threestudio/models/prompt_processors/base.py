...
class PromptProcessorOutput:
    ...
    text_embeddings_aux: Float[Tensor, "N Nf"]
    uncond_text_embeddings_aux: Float[Tensor, "N Nf"]
    text_embeddings_vd_aux: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_vd_aux: Float[Tensor, "Nv N Nf"]
    ...
    def get_multi_text_embeddings(
        ...
        if view_dependent_prompting:
            ...
            text_embeddings = self.text_embeddings_vd[direction_idx]  # type: ignore
            uncond_text_embeddings = self.uncond_text_embeddings_vd[direction_idx]  # type: ignore
            text_embeddings_aux = self.text_embeddings_vd_aux[direction_idx]  # type: ignore
            uncond_text_embeddings_aux = self.uncond_text_embeddings_vd_aux[direction_idx]  # type: ignore            
        else:
            text_embeddings = self.text_embeddings.expand(batch_size, -1, -1)  # type: ignore
            uncond_text_embeddings = self.uncond_text_embeddings.expand(  # type: ignore
                batch_size, -1, -1
            )
            text_embeddings_aux = self.text_embeddings_aux.expand(batch_size, -1, -1)  # type: ignore
            uncond_text_embeddings_aux = self.uncond_text_embeddings_aux.expand(  # type: ignore
                batch_size, -1, -1
            )

        # IMPORTANT: we return (cond, uncond), which is in different order than other implementations!
        return torch.cat([text_embeddings, uncond_text_embeddings], dim=0), torch.cat([text_embeddings_aux, uncond_text_embeddings_aux], dim=0)
...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        # import pdb; pdb.set_trace()
        if self.cfg.multiple_encoders is False:
            self.prepare_text_embeddings()
            self.load_text_embeddings()
        else:
            # import pdb; pdb.set_trace()
            self.prepare_text_embeddings(self.cfg.pretrained_model_name_or_path)
            self.prepare_text_embeddings(self.cfg.pretrained_model_name_or_path_aux)
            self.load_multi_text_embeddings()  
            # import pdb; pdb.set_trace()          
    ...
    @rank_zero_only
    def prepare_text_embeddings(self, pretrained_model_name_or_path = None):
    ...
    def load_text_embeddings(self):
        ...
        self.text_embeddings_aux = None
        self.uncond_text_embeddings_aux = None
        self.text_embeddings_vd_aux = None
        self.uncond_text_embeddings_vd_aux = None
        
        threestudio.debug(f"Loaded text embeddings.")
    
    
    def load_multi_text_embeddings(self):
        # synchronize, to ensure the text embeddings have been computed and saved to cache
        barrier()
        self.text_embeddings = self.load_from_cache(self.prompt)[None, ...]
        # import pdb; pdb.set_trace()
        ...
        # import pdb; pdb.set_trace()
        self.uncond_text_embeddings_aux = self.load_from_cache(self.negative_prompt, self.cfg.pretrained_model_name_or_path_aux)[
            None, ...
        ]
    ...
    def load_from_cache(self, prompt, pretrained_model_name_or_path = None):
